...
export const dusunApi = {
  ...
  async getById(id) {
    if (!id) return null;
    try { return await apiFetch(`${baseUrl}/ref-dusun/${id}`); }
    catch { return null; }
  },

  async create(payload) {
    return apiFetch(`${baseUrl}/ref-dusun`, { method: 'POST', body: JSON.stringify(payload) });
  },

  async update(id, payload) {
    return apiFetch(`${baseUrl}/ref-dusun/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
  },
