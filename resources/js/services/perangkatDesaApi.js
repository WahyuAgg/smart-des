...
export const perangkatDesaApi = {
  ...
  async getById(id) {
    if (!id) return null;
    try { return await apiFetch(`${baseUrl}/ref-perangkat-desa/${id}`); }
    catch { return null; }
  },

  async create(payload) {
    return apiFetch(`${baseUrl}/ref-perangkat-desa`, { method: 'POST', body: JSON.stringify(payload) });
  },

  async update(id, payload) {
    return apiFetch(`${baseUrl}/ref-perangkat-desa/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
  },
