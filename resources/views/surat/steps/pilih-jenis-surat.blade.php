        ...
        <template x-for="item in jenisSuratList" :key="item.id">
            <button type="button" @click="pilihJenisSurat(item)"
                ...
                <div class="flex items-start justify-between gap-2">
                    <div>
                        <p class="text-sm font-semibold text-slate-800 group-hover:text-accent-hover"
                            x-text="item.nama_jenis_surat"></p>
                        <p class="text-xs text-slate-500 mt-1" x-text="item.kode_jenis_surat"></p>
                    </div>
                    <svg class="w-4 h-4 text-slate-300 group-hover:text-accent shrink-0 mt-1" viewBox="0 0 24 24"
