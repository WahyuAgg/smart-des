{{--
  Toolbar generik untuk halaman master-data.
  Mencakup breadcrumb, judul, deskripsi, search input, dan tombol tambah.

  Props:
    title             : Judul halaman (wajib)
    description       : Deskripsi singkat (wajib)
    searchPlaceholder : Placeholder input pencarian
    buttonLabel       : Label tombol tambah
    searchWidth       : Kelas lebar input (default: sm:w-80)
--}}
@props([
    'title',
    'description',
...
<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
  <div>
    ...
    <h1 class="text-2xl font-semibold text-slate-900 mt-1">{{ $title }}</h1>
    <p class="text-sm text-slate-500 mt-1">{{ $description }}</p>
  </div>

  <div class="flex flex-col sm:flex-row gap-3 sm:items-center w-full lg:w-auto">
    <div class="relative w-full {{ $searchWidth }}">
