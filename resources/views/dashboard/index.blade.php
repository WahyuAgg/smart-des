...
@section('content')
  <div x-data="dashboard" class="-mx-6 -mt-6">
    ...
    <div x-show="error" x-cloak class="flex flex-col items-center justify-center py-20 text-center">
      <svg class="w-16 h-16 text-red-300 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        stroke-width="1.5">
      ...
      <button @click="fetchAll()"
        class="mt-4 px-4 py-2 bg-accent text-white rounded-lg text-sm hover:bg-accent-hover transition">
        Coba Lagi
      </button>
    </div>
        ...
        <div class="px-6 py-6 space-y-6">
          ...
          <div class="grid grid-cols-1 xl:grid-cols-4 gap-5">
            ...
            <div class="xl:col-span-3 grid grid-cols-1 xl:grid-cols-3 gap-5">
              ...
              @include('dashboard.partials.wilayah-administrasi')
              @include('dashboard.partials.perangkat-desa')
            </div>
          </div>
          <div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
            @include('dashboard.partials.riwayat-surat')
            @include('dashboard.partials.peminjaman-inventaris')
          </div>
        </div>
