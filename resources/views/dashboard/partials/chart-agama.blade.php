    ...
    <div class="space-y-3">
      <template x-for="(item, index) in dashboard.distribusi_agama" :key="item.agama">
        <div class="flex items-center gap-3 group">
          ...
          <div class="w-28 sm:w-36 h-3 bg-slate-100 rounded-full overflow-hidden shrink-0">
            <div class="h-full rounded-full transition-all duration-700 ease-out group-hover:brightness-110"
                 :style="`width: ${barPct(item.jumlah, maxVal(dashboard.distribusi_agama))}%; background-color: ${color(index)}`">
            </div>
          </div>
