    ...
    <div class="space-y-2 max-h-85 overflow-y-auto pr-1">
      <template x-for="(item, index) in perangkatDesa" :key="item.jabatan_perangkat?.kode || index">
        <div class="flex items-center gap-3 p-2.5 rounded-lg hover:bg-slate-50 transition group">
          {{-- Avatar --}}
          <div
            class="w-9 h-9 rounded-full shrink-0 flex items-center justify-center text-white text-xs font-bold shadow-sm bg-linear-to-br"
            :class="[
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-slate-700 truncate" x-text="item.nama || '-'"></p>
            <p class="text-[11px] text-slate-400 truncate" x-text="item.jabatan_perangkat?.nama || '-'"></p>
          </div>
          {{-- Badge --}}
          <template x-if="(item.jabatan_perangkat?.urutan ?? 999) <= 2">
            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full"
              :class="(item.jabatan_perangkat?.urutan ?? 999) === 1 ? 'bg-yellow-100 text-yellow-700' :
                  'bg-blue-100 text-blue-700'"
              x-text="(item.jabatan_perangkat?.urutan ?? 999) === 1 ? 'KADES' : 'SEKDES'"></span>
          </template>
        </div>
      </template>
    </div>
