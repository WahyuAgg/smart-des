<div x-show="stockAction === 'pengadaan'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
  <div x-show="stockAction === 'pengadaan'" x-transition.opacity @click="closeStockAction()" class="absolute inset-0 bg-slate-900/40"></div>
  <div x-show="stockAction === 'pengadaan'" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative bg-white rounded-xl shadow-lg w-full max-w-md p-5">
    <h3 class="text-sm font-semibold text-slate-800 mb-1">Pengadaan Stok</h3>
    <p class="text-xs text-slate-500 mb-4">Tambah stok untuk <strong x-text="stockItem?.nama_barang"></strong></p>
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <textarea x-model="stockForm.keterangan" placeholder="Opsional" rows="2"
