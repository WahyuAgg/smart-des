        ...
        <template x-for="item in items" :key="item.id">
          <tr class="hover:bg-slate-50">
            <td class="px-4 py-3 text-slate-600">
              <div x-text="item.nik || '—'"></div>
<div class="text-xs text-slate-400"
     x-text="'KK: ' + (item.kk?.no_kk || '-')">
</div>            </td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3">
              <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium"
                :class="statusBadge(item.status_hidup)" x-text="statusLabel(item.status_hidup)"></span>
            </td>
            ...
            <td class="px-4 py-3">
              <div class="flex items-center justify-end gap-3">
                <button type="button" @click="openDetail(item)" class="text-slate-400 hover:text-accent" title="Detail">
