...
<div x-data="masterFieldSurat()" x-init="init()" class="max-w-5xl mx-auto">
  ...
  <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <template x-if="!loading && items.length === 0">
            <tr>
              <td colspan="6" class="px-4 py-10 text-center text-slate-400">Belum ada data field surat.</td>
            </tr>
          </template>
    ...
    {{-- pagination --}}
    <div x-show="meta.last_page > 1" class="flex items-center justify-between px-4 py-3 border-t border-slate-200 text-xs text-slate-500">
      <span>Halaman <span x-text="meta.current_page"></span> dari <span x-text="meta.last_page"></span> &middot; <span x-text="meta.total"></span> data</span>
  ...
  <x-modal max-width="max-w-xl">
    ...
    <form @submit.prevent="save()" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <x-form.input label="Nama (key)" model="form.nama" placeholder="cth: nama_pelapor" required hint="Dipakai sebagai placeholder di template, tanpa spasi." />
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <x-form.input label="Source Field" model="form.source_field" placeholder="cth: nik"
                      x-show="form.source" hint="Nama kolom pada sumber data di atas." />
      </div>
...
@push('scripts')
  <script>
    window.API_BASE_URL = window.API_BASE_URL || "{{ rtrim(config('services.surat.base_url', env('SURAT_API_URL', url('/api'))), '/') }}";
  </script>
