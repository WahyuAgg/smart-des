    ...
    <table class="w-full text-sm">
      ...
      <tbody class="divide-y divide-slate-100">
        <template x-if="!loading && items.length === 0">
          <tr>
            <td colspan="6" class="px-4 py-10 text-center text-slate-400">Belum ada data jabatan perangkat.</td>
          </tr>
        </template>
        ...
        <template x-for="(item, index) in items" :key="item.id">
          <tr class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 font-mono text-xs text-slate-500" x-text="item.kode"></td>
            <td class="px-4 py-3 font-medium text-slate-800" x-text="item.nama"></td>
            <td class="px-4 py-3 text-slate-600" x-text="item.urutan"></td>
            <td class="px-4 py-3">
