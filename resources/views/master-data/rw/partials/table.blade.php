    ...
    <table class="w-full text-sm">
      <thead>
        <tr class="bg-slate-50 border-b border-slate-200 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">
          ...
          <th class="px-4 py-3">Nomor RW</th>
          <th class="px-4 py-3">Ketua RW</th>
      ...
      <tbody class="divide-y divide-slate-100">
        <template x-if="!loading && items.length === 0">
          <tr>
            <td colspan="5" class="px-4 py-10 text-center text-slate-400">Belum ada data RW.</td>
          </tr>
        </template>
        ...
        <template x-for="(item, index) in items" :key="item.id">
          <tr class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-slate-600" x-text="dusunName(item.dusun_id)"></td>
            <td class="px-4 py-3 font-medium text-slate-800" x-text="item.nomor_rw"></td>
            <td class="px-4 py-3 text-slate-600" x-text="item.ketua_rw || '—'"></td>
            <td class="px-4 py-3">
